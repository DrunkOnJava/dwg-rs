...
`dwg-rs` is not a substitute for the ODA SDK. It is a complement, aimed specifically at the Apache-2-only segment — projects that cannot take the SDK license but still need interoperability. Feedback from ODA members is welcome.
