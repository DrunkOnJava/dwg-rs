...
/// P0-12: DXF output must be byte-for-byte deterministic across two
/// back-to-back conversions of the same input. No embedded system
/// timestamps, no ordering via `HashMap` iteration, no nondeterministic
/// padding. Catches regressions where a writer accidentally injects
/// `chrono::Local::now()` or similar.
#[test]
fn dxf_output_is_deterministic() {
    ...
    // Also assert: the output contains no 4-digit year that matches
    // the current year, to catch an accidental strftime leak.
